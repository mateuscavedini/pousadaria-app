...
describe 'Proprietário edita um quarto' do
  it 'de uma pousada que não é sua' do
    ...
    login_as maria
    patch guesthouse_room_path(jose_guesthouse, jose_room), params: { room: { description: 'Nova descrição para o quarto' } }

    expect(response).to redirect_to root_path
  end
  
  it 'não é de sua pousada' do
    maria = Owner.create!(name: 'Maria', email: 'maria@email.com', password: 'senha123')
    ...
    login_as maria
    patch guesthouse_room_path(maria_guesthouse, jose_room), params: { room: { description: 'Nova descrição' } }
