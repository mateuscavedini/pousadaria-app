...
describe 'Proprietário edita dados de quarto' do
  it 'e deve estar autenticado' do
    ...
    visit edit_guesthouse_room_path(guesthouse, room)
  ...
  it 'de pousada que não é sua' do
    ...
    login_as maria
    visit edit_guesthouse_room_path(jose_guesthouse, jose_room)

    expect(current_path).to eq root_path
    expect(page).to have_content 'Você não tem acesso a esse recurso.'
  end

  it 'que não pertence a sua pousada' do
    maria = Owner.create!(name: 'Maria', email: 'maria@email.com', password: 'senha123')
    ...
    login_as maria
    visit edit_guesthouse_room_path(maria_guesthouse, jose_room)

    expect(current_path).to eq root_path
    expect(page).to have_content 'Este recurso não existe.'
  end
  ...
  it 'com sucesso' do
    ...
    click_on 'Editar'
    fill_in 'Diária', with: '75.90'
    check 'Possui Varanda?'
    uncheck 'Possui Cofre?'
    click_on 'Enviar'

    expect(page).to have_content 'Quarto atualizado com sucesso.'
    expect(page).to have_content 'Diária: R$ 75,90'
    expect(page).to have_content 'Possui Varanda? Sim'
